...
{
    public class OutboundTransportSubcriptionHandler : IRequestHandler<OutboundTransportSubcriptionExtCommand, WebhookSubsciptionsPatchResponse>
    {
        private readonly IWebhookSubscriptionFactory webhookFactory;
        private readonly ILogger<OutboundTransportSubcriptionHandler> logger;
        private readonly ISubscriptionExtensionLogRepository subcriptionRepository;

        public OutboundTransportSubcriptionHandler(IWebhookSubscriptionFactory webhookFactory, ILogger<OutboundTransportSubcriptionHandler> logger, ISubscriptionExtensionLogRepository subcriptionRepository)
        {
        ...
        public async Task<WebhookSubsciptionsPatchResponse> Handle(OutboundTransportSubcriptionExtCommand request, CancellationToken cancellationToken)
        {
            ...
            var result = await webhookService.ExtendSubscription(request.subsciptionsParams.Item1, Enums.WebhookSubscriptionType.TransportOutbound, request.subsciptionsParams.Item4);
