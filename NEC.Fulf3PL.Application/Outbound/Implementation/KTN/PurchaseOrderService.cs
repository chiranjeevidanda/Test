...
{
    public class PurchaseOrderService : IPurchaseOrderService<OutboundConverterModel>
    {
        private readonly ILogger<PurchaseOrderService> logger;
        private readonly IConfiguration configuration;
        private readonly IRequestHandler requestHandler;
        private readonly IOutboundTransactionRepository outboundTransactionRepository;

        public PurchaseOrderService(ILogger<PurchaseOrderService> logger, IConfiguration configuration, IRequestHandler requestHandler, IOutboundTransactionRepository outboundTransactionRepository)
        {
            this.logger = logger;
        ...
        public async Task<ExecuteResult<bool>> PostData(OutboundConverterModel convertedData)
        {
            ...
            if (endpointType.Equals(Enums.OutboundAPIEndpoint.PurchaseOrder))
            {
                ...
                externalRequest = await CreateAPIRequest(endpointURL: configuration["KtnPurchaseOrderAPI"], body: JsonConvert.SerializeObject(purchaseOrderPayload), endpointType: endpointType, convertedData: convertedData, false, purchaseOrderPayload.Customer);
            }
        ...
        public async Task<ExecuteResult<bool>> PutData(OutboundConverterModel convertedData, string systemId)
        {
            ...
            if (endpointType.Equals(Enums.OutboundAPIEndpoint.PurchaseOrder))
            {
                ...
                externalRequest = await CreateAPIRequest(endpointURL: string.Format(configuration["KtnUpdatePOAPI"], systemId), body: JsonConvert.SerializeObject(purchaseOrderPayload), endpointType: endpointType, convertedData: convertedData, true, purchaseOrderPayload.Customer);
            }
            ...
            if (message != null)
            {
                ...
                if (blobResponse != null)
                {
                    ...
                    if (Enum.TryParse<Enums.OutboundAPIEndpoint>(blobResponse.InboundEndpoint, true, out endpointType))
                    {
                        ...
                        externalRequest.URL = configuration["KtnPurchaseOrderConverter"];
