...
{
    public class ReturnOrderService : IReturnOrderService<OutboundConverterModel>
    {
        private readonly ILogger<ReturnOrderService> logger;
        private readonly IConfiguration configuration;
        private readonly IRequestHandler requestHandler;
        private readonly IOutboundTransactionRepository outboundTransactionRepository;

        public ReturnOrderService(ILogger<ReturnOrderService> logger, IConfiguration configuration, IRequestHandler requestHandler, IOutboundTransactionRepository outboundTransactionRepository)
        {
            this.logger = logger;
        ...
        public async Task<ExecuteResult<bool>> PostData(OutboundConverterModel convertedData)
        {
            ...
            if (endpointType.Equals(Enums.OutboundAPIEndpoint.ReturnOrder))
            {
                ...
                externalRequest = await CreateAPIRequest(endpointURL: configuration["KtnReturnOrderAPI"], body: JsonConvert.SerializeObject(returnOrderPayload), endpointType: endpointType, convertedData: convertedData, false, returnOrderPayload.Customer);
            }
        ...
        public async Task<ExecuteResult<bool>> PutData(OutboundConverterModel convertedData, string systemId)
        {
            ...
            if (endpointType.Equals(Enums.OutboundAPIEndpoint.ReturnOrder))
            {
                ...
                externalRequest = await CreateAPIRequest(endpointURL: string.Format(configuration["KtnUpdateReturnAPI"], systemId), body: JsonConvert.SerializeObject(returnOrderPayload), endpointType: endpointType, convertedData: convertedData, true, returnOrderPayload.Customer);
            }
        ...
        public async Task<ExecuteResult<OutboundConverterModel>> MapData(string request)
        {
            ...
            if (message != null)
            {
                ...
                if (Enum.TryParse<Enums.OutboundAPIEndpoint>(blobResponse.InboundEndpoint, true, out endpointType))
                {
                    ...
                    externalRequest.URL = configuration["KtnReturnOrderConverter"];
