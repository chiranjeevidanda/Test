...
{
    public class ReturnOrderServiceCommon : IReturnOrderServiceCommon<OutboundConverterModel>
    {
        private readonly ILogger<ReturnOrderServiceCommon> logger;
        private readonly IConfiguration configuration;
        private readonly IRequestHandler requestHandler;
        private readonly IOutboundTransactionRepository outboundTransactionRepository;

        public ReturnOrderServiceCommon(ILogger<ReturnOrderServiceCommon> logger, IConfiguration configuration, IRequestHandler requestHandler, IOutboundTransactionRepository outboundTransactionRepository)
        {
            this.logger = logger;
        ...
        public async Task<ExecuteResult<bool>> PostData(OutboundConverterModel convertedData)
        {
            ...
            if (endpointType.Equals(Enums.OutboundAPIEndpoint.ReturnOrder))
            {
                ReturnOrderDTO returnOrderPayload = JsonConvert.DeserializeObject<ReturnOrderDTO>(convertedData.ConvertedData);

                if (returnOrderPayload == null)
                {
                    throw new InternalServierException(Constants.ErrorOccured);
                }

                externalRequest = await CreateAPIRequest(endpointURL: configuration["GeodisOutboundAPI"], body: JsonConvert.SerializeObject(returnOrderPayload), endpointType: endpointType, convertedData: convertedData, false, returnOrderPayload.Customer);
            }
        ...
        public async Task<ExecuteResult<bool>> PutData(OutboundConverterModel convertedData, string systemId)
        {
            ...
            if (endpointType.Equals(Enums.OutboundAPIEndpoint.ReturnOrder))
            {
                ...
                externalRequest = await CreateAPIRequest(endpointURL: string.Format(configuration["GeodisOutboundAPI"], systemId), body: JsonConvert.SerializeObject(returnOrderPayload), endpointType: endpointType, convertedData: convertedData, true, returnOrderPayload.Customer);
            }
        ...
        public async Task<ExecuteResult<OutboundConverterModel>> MapData(string request)
        {
            ...
            logger.LogInformation("Mapping Return Order Data.....");
            ...
            if (message != null)
            {
                ...
                OutboundDeliveryMessage blobResponse = new OutboundDeliveryMessage();
                blobResponse.InboundEndpoint = Enums.OutboundAPIEndpoint.ReturnOrder.ToString();
                blobResponse.ProcessId = Guid.NewGuid().ToString();
                blobResponse.RequestData = response.Result;
                ...
                if (Enum.TryParse<Enums.OutboundAPIEndpoint>(blobResponse.InboundEndpoint, true, out endpointType))
                {
                    ...
                    externalRequest.URL = configuration["GeodisReturnOrderConverter"];
