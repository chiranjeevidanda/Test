...
{
    public class CreateOrderServiceCommon : ICreateOrderServiceCommon<OutboundConverterModel>
    {
        private readonly ILogger<CreateOrderServiceCommon> logger;
        private readonly IConfiguration configuration;
        private readonly IRequestHandler requestHandler;
        private readonly IOutboundTransactionRepository outboundTransactionRepository;

        public CreateOrderServiceCommon(ILogger<CreateOrderServiceCommon> logger, IConfiguration configuration, IRequestHandler requestHandler, IOutboundTransactionRepository outboundTransactionRepository)
        {
            this.logger = logger;
        ...
        public async Task<ExecuteResult<bool>> PostData(OutboundConverterModel convertedData)
        {
            ...
            if (endpointType.Equals(Enums.OutboundAPIEndpoint.CreateOrder))
            {
                ...
                externalRequest = await CreateAPIRequest(endpointURL: configuration["GeodisOutboundAPI"], body: JsonConvert.SerializeObject(createOrderPayload), endpointType: endpointType, convertedData: convertedData, false, createOrderPayload.Customer);
            }
        ...
        public async Task<ExecuteResult<bool>> PutData(OutboundConverterModel convertedData, string systemId)
        {
            ...
            if (endpointType.Equals(Enums.OutboundAPIEndpoint.CreateOrder))
            {
                ...
                externalRequest = await CreateAPIRequest(endpointURL: string.Format(configuration["GeodisOutboundAPI"], systemId), body: processData, endpointType: endpointType, convertedData: convertedData, true, createOrderPayload.Customer);
            }
            ...
            if (message != null)
            {
                ...
                if (blobResponse != null)
                {
                    ...
                    if (Enum.TryParse<Enums.OutboundAPIEndpoint>(blobResponse.InboundEndpoint, true, out endpointType))
                    {
                        ...
                        externalRequest.URL = configuration["GeodisCreateOrderConverter"];
