...
using NEC.Fulf3PL.Application.Inbound.Interface.GEODIS;
using static NEC.Fulf3PL.Application.Inbound.Implementation.GEODIS.Mapper.DeliveryProcessMapper;
using NEC.Fulf3PL.Application.Inbound.Implementation.COMMON;
using NEC.Fulf3PL.Application.Inbound.Implementation.COMMON.Model;

namespace NEC.Fulf3PL.Application.Inbound.Implementation.GEODIS
{
    public class GeodisInboundService : IGeodisInboundService<InboundConverterModel>
    {
        private readonly ILogger<GeodisInboundService> logger;
        private readonly IRequestHandler requestHandler;
        private readonly IInboundTransactionRepository inboundTransactionRepository;

        public GeodisInboundService(ILogger<GeodisInboundService> logger, IRequestHandler requestHandler, IInboundTransactionRepository inboundTransactionRepository)
        {
            this.logger = logger;
        ...
        public async Task<ExecuteResult<InboundConverterModel>> GetData(InboundConverterModel convertedData, string requestURL)
        {
            ...
            externalRequest = await CreateAPIRequest(endpointURL: convertedData.URL, endpointType: endpointType, convertedData: convertedData);
            ...
            else if (!providerResponse.Success)
            {
                ...
                await LogPostProcess(convertedData.ProcessId, convertedData.Endpoint.ToString(), externalRequest, providerResponse, "Failed while getting data from GEODIS API");
        ...
        private async Task<ExternalRequest> CreateAPIRequest(string endpointURL, Enums.InboundAPIEndpoint endpointType, InboundConverterModel convertedData)
        {
        ...
        private async Task<ExternalRequest> CreatePostAPIRequest(string endpointURL, Enums.InboundAPIEndpoint endpointType, InboundConverterModel requestData)
        {
            var result = new ExternalRequest();
            result.URL = endpointURL;
        ...
        private async Task<Dictionary<string, string>> GetHeader(Enums.InboundAPIEndpoint key, string companyId)
        {
            ...
            var headerParamsAll = DeliveryProcessHelper.LoadGeodisHeaderParams(companyId);
        ...
        public async Task<ExecuteResult<InboundConverterModel>> PostData(InboundConverterModel requestData, string requestURL)
        {
            ...
            externalRequest = await CreatePostAPIRequest(endpointURL: requestURL, endpointType: endpointType, requestData: requestData);
            ...
            {
                //log in db
                await LogPostProcess(requestData.ProcessId, requestData.Endpoint.ToString(), externalRequest, providerResponse, "Failed while fetching converter API response");
                throw;
            }
            if (providerResponse.Success && providerResponse.Result != null)
            {
        ...
        public async Task<ExecuteResult<InboundConverterModel>> MapData(string request, string requestURL)
        {
            var result = new ExecuteResult<InboundConverterModel>();

            logger.LogInformation("Mapping Inventory Data.....");
