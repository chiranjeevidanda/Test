...
{
    public class InventoryAdjustment : IInventoryAdjustment<InboundConverterModel>
    {
        private readonly ILogger<InventoryAdjustment> logger;
        private readonly IConfiguration configuration;
        private readonly IRequestHandler requestHandler;
        private readonly IInboundTransactionRepository inboundTransactionRepository;

        public InventoryAdjustment(ILogger<InventoryAdjustment> logger, IConfiguration configuration, IRequestHandler requestHandler, IInboundTransactionRepository inboundTransactionRepository)
        {
            this.logger = logger;
        ...
        public async Task<ExecuteResult<InboundConverterModel>> PostData(InboundConverterModel requestData, string requestURL)
        {
            ...
            //configuration["KtnInventorySapBapiURL"]
        ...
        public async Task<ExecuteResult<InboundConverterModel>> MapData(string request)
        {
            ...
            if (message != null)
            {
                ...
                if (Enum.TryParse<Enums.InboundAPIEndpoint>(message.InboundEndpoint, true, out endpointType))
                {
                    ...
                    if (endpointType.Equals(Enums.InboundAPIEndpoint.Inventory))
                    {
                        InventoryDetailsModel receiptData = JsonConvert.DeserializeObject<InventoryDetailsModel>(message.RequestData);
                        externalRequest.URL = configuration["KtnInventoryConverter"];
                        result.Result.TotalRecords = 1;
