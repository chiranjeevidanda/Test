...
{
    public class ReturnSubcriptionHandler : IRequestHandler<ReturnSubcriptionExtCommand, WebhookSubsciptionsPatchResponse>
    {
        private readonly IWebhookSubscriptionFactory webhookFactory;
        private readonly ILogger<ReturnSubcriptionHandler> logger;
        private readonly ISubscriptionExtensionLogRepository subcriptionRepository;

        public ReturnSubcriptionHandler(IWebhookSubscriptionFactory webhookFactory, ILogger<ReturnSubcriptionHandler> logger, ISubscriptionExtensionLogRepository subcriptionRepository)
        {
        ...
        public async Task<WebhookSubsciptionsPatchResponse> Handle(ReturnSubcriptionExtCommand request, CancellationToken cancellationToken)
        {
            ...
            var result = await webhookService.ExtendSubscription(request.subsciptionsParams.Item1, Enums.WebhookSubscriptionType.ReturnReceipt, request.subsciptionsParams.Item4);
