...
{
    public class GoodsReceivedPO : IGoodsReceivedPO<InboundConverterModel>
    {
        private readonly ILogger<GoodsReceivedPO> logger;
        private readonly IConfiguration configuration;
        private readonly IRequestHandler requestHandler;
        private readonly IInboundTransactionRepository inboundTransactionRepository;

        public GoodsReceivedPO(ILogger<GoodsReceivedPO> logger, IConfiguration configuration, IRequestHandler requestHandler, IInboundTransactionRepository inboundTransactionRepository)
        {
            this.logger = logger;
        ...
        public async Task<ExecuteResult<InboundConverterModel>> PostData(InboundConverterModel requestData)
        {
            var result = new ExecuteResult<InboundConverterModel>();
            var providerResponse = new ExecuteResult<string>();
            ...
            externalRequest = await CreatePostAPIRequest(endpointURL: configuration["KtnGoodsReceivedSapBapiURL"], endpointType: endpointType, requestData: requestData, companyId);
        ...
        public async Task<ExecuteResult<InboundConverterModel>> MapData(string request)
        {
            ...
            logger.LogInformation("Mapping Goods Received Data.....");
            ...
            if (message != null)
            {
                ...
                if (Enum.TryParse<Enums.InboundAPIEndpoint>(message.InboundEndpoint, true, out endpointType))
                {
                    ...
                    if (endpointType.Equals(Enums.InboundAPIEndpoint.TransportInbounds))
                    {
                        GoodsReceivedDetailsModel receiptData = JsonConvert.DeserializeObject<GoodsReceivedDetailsModel>(message.RequestData);
                        externalRequest.URL = configuration["KtnGoodsReceivedConverter"];
                        result.Result.TotalRecords = 1;
