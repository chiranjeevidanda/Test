...
{
    public class GoodsIssuedOrder : IGoodsIssuedOrder<InboundConverterModel>
    {
        private readonly ILogger<GoodsIssuedOrder> logger;
        private readonly IConfiguration configuration;
        private readonly IRequestHandler requestHandler;
        private readonly IInboundTransactionRepository inboundTransactionRepository;

        public GoodsIssuedOrder(ILogger<GoodsIssuedOrder> logger, IConfiguration configuration, IRequestHandler requestHandler, IInboundTransactionRepository inboundTransactionRepository)
        {
            this.logger = logger;
        ...
        public async Task<ExecuteResult<InboundConverterModel>> PostData(InboundConverterModel requestData, string requestURL)
        {
            ...
            {
                //log in db
                await LogPostProcess(requestData.ProcessId, requestData.Endpoint.ToString(), externalRequest, providerResponse, "Failed while posting the data to API");
                throw;
            }
            if (providerResponse.Success && providerResponse.Result != null)
            {
        ...
        public async Task<ExecuteResult<InboundConverterModel>> MapData(string request)
        {
            ...
            logger.LogInformation("Mapping Goods Issued Data.....");
            ...
            if (message != null)
            {
                ...
                if (Enum.TryParse<Enums.InboundAPIEndpoint>(message.InboundEndpoint, true, out endpointType))
                {
                    ...
                    if (endpointType.Equals(Enums.InboundAPIEndpoint.TransportOutbounds))
                    {
                        OutboundShipmentDetails receiptData = JsonConvert.DeserializeObject<OutboundShipmentDetails>(message.RequestData);
                        externalRequest.URL = configuration["KtnGoodsIssuedConverter"];
                        result.Result.TotalRecords = 1;
