...
{
    public class OutboundWebSubscription : WebhookSubscription, IWebhookSubscription
    {
        private readonly ILogger<WebhookSubscription> logger;

        public OutboundWebSubscription(ILogger<WebhookSubscription> logger, IConfiguration configuration, ITokenGenerationService<GenerateTokenDTO> iTokenGenerationService, IRequestHandler requestHandler)
        ...
        public override async Task<ExecuteResult<WebhookSubsciptionsPatchResponse>> ExtendSubscription(WebhookSubscriptionData subscription, Enums.WebhookSubscriptionType key, string customer = Constants.NEWERAB2B)
        {
            ...
            logger.LogInformation($"Successfully extended {nameof(OutboundWebSubscription)}");
            return result;
        }
